...
def run_training(cfg: Dict[str, Any]) -> None:
    ...
    shuffle_train = _as_bool(cfg.get("shuffle_train"), True)
    shuffle_eval = _as_bool(cfg.get("shuffle_eval"), False)
    ...
    if num_epochs <= 0:
        ...
        for epoch in range(1, num_epochs + 1):
            train_metrics = train_one_epoch(
                ...
                max_batches=int(limit_train_batches) if limit_train_batches else None,
            )
            ...
            if val_loader is not None:
                val_metrics = evaluate_model(
                    ...
                    device,
                    pad_id=pad_id,
                    max_batches=int(limit_val_batches) if limit_val_batches else None,
                )
    ...
    if test_loader is not None:
        ...
        test_metrics = evaluate_model(
            ...
            device,
            pad_id=pad_id,
            max_batches=int(limit_test_batches) if limit_test_batches else None,
        )
...
def cmd_overfit(args: argparse.Namespace) -> None:
    """Entry point for the legacy ``overfit`` helper command.

    It keeps compatibility with :mod:`scripts.sanity_overfit` by forcing a
    single-batch training loop with conservative defaults that favour fast
    convergence (no dropout/weight decay, deterministic shuffling, limited
    validation/testing).  The ``--steps`` flag maps to the number of epochs to
    ...
    if steps is None or int(steps) <= 0:
        steps = int(cfg.get("num_epochs", 50) or 50)
    steps = int(steps)

    cfg.setdefault("dropout", 0.0)
    cfg["weight_decay"] = 0.0
    cfg.setdefault("shuffle_train", False)
    cfg.setdefault("shuffle_eval", False)
    cfg["limit_train_batches"] = 1
    if cfg.get("limit_val_batches") is None:
        cfg["limit_val_batches"] = 1
    if cfg.get("limit_test_batches") is None:
        cfg["limit_test_batches"] = 1
    cfg["num_epochs"] = steps

    LOGGER.info(
        "Modalità overfit: num_epochs=%d limit_train_batches=1 limit_val_batches=%s",
        steps,
        cfg.get("limit_val_batches"),
    )
...
def run_evaluation(cfg: Dict[str, Any], checkpoint: str, splits: list[str]) -> None:
    ...
    for split in splits:
        ...
        loader = build_dataloader(
            ...
            tokenizer,
            batch_size=batch_size,
            shuffle=_as_bool(cfg.get("shuffle_eval"), False),
            num_workers=num_workers,
        )
        limit_eval_batches = cfg.get("limit_eval_batches") or cfg.get("limit_val_batches")
        metrics = evaluate_model(
            model,
            loader,
            device,
            pad_id=pad_id,
            max_batches=int(limit_eval_batches) if limit_eval_batches else None,
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_overfit = sub.add_parser(
        "overfit",
        help="Forza l'overfit su un singolo batch (compatibilità con gli script legacy)",
    )
    ...
    p_overfit.add_argument(
        ...
        type=int,
        default=50,
        help="Numero di epoche sul singolo batch (default: 50)",
    )
