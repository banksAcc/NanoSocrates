...
def _batch_exact_match(
    outputs: Dict[str, object], labels: torch.Tensor, *, pad_id: int
) -> float:
    ...
    per_example = _per_example_exact(logits, labels, pad_id=pad_id)
...
def train_one_epoch(
    ...
    *,
    ...
    for step, batch in enumerate(loader, start=1):
        ...
        total_exact += _batch_exact_match(outputs, batch["labels"], pad_id=pad_id)
        total_batches += 1
    ...
    *,
    ...
    with torch.no_grad():
        for step, batch in enumerate(loader, start=1):
            ...
            batch_exact = _batch_exact_match(outputs, batch["labels"], pad_id=pad_id)
            ...
            if per_example is not None and tasks:
