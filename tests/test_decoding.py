...
def test_rdf_grammar_masks_invalid_transitions():
    ...
    assert tokens[0] == "<SOT>"
    assert tokens[1] == "<SUBJ>"
    assert text.strip().startswith("<SOT> <SUBJ>"), "the decoded string should expose the structural prefix"
    triples = parse_rdf(" ".join(tokens))
...
def test_structured_decoding_retains_leading_sot_marker():
    vocab = [
        "<pad>",
        "<SOT>",
        "<EOT>",
    ...
    model = DummyModel(
        ...
        transitions={
            ...
            (1, 3, 6, 4, 7, 5): {8: 5.0},
            (1, 3, 6, 4, 7, 5, 8): {2: 5.0},
            (1, 3, 6, 4, 7, 5, 8, 2): {1: 5.0},
            (1, 3, 6, 4, 7, 5, 8, 2, 1): {3: 5.0},
            (1, 3, 6, 4, 7, 5, 8, 2, 1, 3): {6: 5.0},
            (1, 3, 6, 4, 7, 5, 8, 2, 1, 3, 6): {4: 5.0},
            (1, 3, 6, 4, 7, 5, 8, 2, 1, 3, 6, 4): {7: 5.0},
            (1, 3, 6, 4, 7, 5, 8, 2, 1, 3, 6, 4, 7): {5: 5.0},
            (1, 3, 6, 4, 7, 5, 8, 2, 1, 3, 6, 4, 7, 5): {8: 5.0},
            (1, 3, 6, 4, 7, 5, 8, 2, 1, 3, 6, 4, 7, 5, 8): {2: 5.0},
        },
    )
    ...
    tokens = _decode_ids(tok, ids)
    assert tokens[0] == "<SOT>"
    assert text.strip().startswith("<SOT> <SUBJ>"), "structured output should retain the opening <SOT> marker"
